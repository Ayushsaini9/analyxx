"""
Generate Analyxx-branded JEE Mains PYQ papers from scraped PDFs.
...
SOURCE_DIR = BASE_DIR / "papers_to_upload" / "JEE Mains"
OUTPUT_DIR = BASE_DIR / "generated_papers" / "jee_mains"
...
JEE_META = {"duration": "3 Hours", "max_marks": "300", "subjects": "Physics · Chemistry · Mathematics"}
...
def parse_jee_filename(filename: str):
    """Parse jee_mains_2024_jan_27jan_shift1.pdf → metadata dict."""
    stem = Path(filename).stem  # jee_mains_2024_jan_27jan_shift1
    year_match = re.search(r'(20\d{2})', stem)
    if not year_match:
        return None
    year = int(year_match.group(1))
    shift_match = re.search(r'shift(\d)', stem)
    shift = int(shift_match.group(1)) if shift_match else None
    session = None
    date_str = None
    for m in MONTH_NAMES:
        if f"_{m}_" in stem or f"_{m}" == stem[-len(m)-1:]:
            session = m
            break
    date_match = re.search(r'(\d{2})(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)', stem)
    if date_match:
        date_str = f"{date_match.group(1)} {MONTH_NAMES.get(date_match.group(2), date_match.group(2))}"
        if not session:
            session = date_match.group(2)

    display = f"JEE Mains {year}"
    if date_str:
        display += f" — {date_str}"
    if shift:
        display += f" Shift {shift}"

    return {
        "year": year, "session": session, "shift": shift,
        "date_str": date_str, "display": display, "stem": stem,
    }
...
def parse_jee_paper(raw_text: str) -> dict:
    """Parse JEE Mains paper text into structured JSON."""
...
def generate_html(structured: dict, meta: dict) -> str:
    """Generate Analyxx-branded HTML for a JEE Mains paper."""
...
<div class="page">
  ...
  <div class="question-header">
    <div class="brand-sm">
      ...
      <div class="nm">Analyxx<span> AI</span> · JEE Mains {year}</div>
    </div>
  ...
  <div class="page-footer">
    ...
    shift_label = f"Shift {shift}" if shift else ""
    subtitle = f"JEE Mains · {date_str} {shift_label} · {year}".strip()
...
<div class="page" id="cover">
  ...
  <div class="cover-header">
    <div class="booklet-box">
      ...
      <span class="code-num">JM</span>
      <div style="font-family:'DM Sans',sans-serif;font-size:7pt;color:var(--light);margin-top:1mm;letter-spacing:.06em;">NTA — JEE</div>
    </div>
  ...
  <div class="exam-title-block">
    <div class="exam-name">JEE Mains — Paper</div>
...
def process_paper(pdf_path: Path, meta: dict, do_upload: bool = False) -> bool:
    ...
    # 5. Upload
    if do_upload:
        # Storage: jee-mains/Paper/{stem_without_jee_mains_}.pdf
        label = stem.replace("jee_mains_", "")
...
def main():
    parser = argparse.ArgumentParser(description="Generate Analyxx-branded JEE Mains papers")
    ...
    if args.file:
        ...
        files = sorted(SOURCE_DIR.glob("jee_mains_*.pdf"))
        if args.year:
    ...
    print(f"\n🚀 Analyxx JEE Mains Template Generator")
